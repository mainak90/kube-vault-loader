...
def main(args=None):
	...
	parser.add_argument("-n", "--namespace", help="[OPTIONAL] The kubernetes and vault namespace to connect to, only use when the --pushconfig is set")
	...
	if args.action and args.out:
		...
		print('output: {output} action: {action}'.format(output=output, action=action))
	else:
		...
		sys.exit(1)
	path = args.path if args.path is not None else os.path.abspath('template/template.yaml')
	...
	if args.pushconfig and args.namespace and args.action == "export":
		...
		secretpath=args.secretpath
		logging.info('Pushconfig has not been requested, if --namespace is provided it will be ignored')
